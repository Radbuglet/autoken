...
pub use self::adapted_encoding::{CacheDecoder, CacheEncoder};
    ...
    D: for<'a> Decodable<CacheDecoder<'a, 'tcx>>,
{
    match adapted_io::read_file(path) {
        ...
        Err(err) => tcx
            .sess
            .dcx()
            .fatal(format!("failed to load {name} to `{path:?}`: {err}")),
    }
}

// === Adapted === //

// Adapted from: compiler/rustc_incremental/src/persist/file_format.rs:23
mod adapted_io {
    use std::{
        fs,
        io::{self, Read},
        ...
        F: FnOnce(FileEncoder) -> FileEncodeResult,
    {
        match fs::remove_file(&path_buf) {
            ...
            Err(err) => {
                sess.dcx().fatal(format!(
                    "unable to delete old {name} at `{path_buf:?}`: {err}"
                ));
            }
        }
        ...
        let mut encoder = match FileEncoder::new(&path_buf) {
            ...
            Err(err) => {
                sess.dcx()
                    .fatal(format!("failed to create {name} at `{path_buf:?}`: {err}"));
            }
        };
        ...
        match encode(encoder) {
            ...
            Err((path, err)) => sess
                .dcx()
                .fatal(format!("failed to write {name} to `{path:?}`: {err}")),
        }
    }
    ...
    pub fn read_file(path: &Path) -> io::Result<Option<(Mmap, usize)>> {
        ...
        // Check FILE_MAGIC
        let mut file_magic = Box::from_iter((0..FILE_MAGIC.len()).map(|_| 0));
...
mod adapted_encoding {
    use std::{collections::hash_map::Entry, mem, sync::Arc};

    use rustc_data_structures::{
        fx::FxIndexSet,
        memmap::Mmap,
        sync::{HashMapExt, Lock},
        unhash::UnhashMap,
        unord::{UnordMap, UnordSet},
    };
    use rustc_hash::FxHashMap;
    use rustc_hir::def_id::{CrateNum, DefId, DefIndex, DefPathHash, LocalDefId, LOCAL_CRATE};
    ...
    use rustc_middle::{
        ...
        mir::{
            self,
            interpret::{self, AllocDecodingSession, AllocDecodingState},
        },
        ty::{self, codec::RefDecodable, Ty, TyCtxt},
    };
    use rustc_serialize::{
    ...
    use rustc_span::{
        hygiene::{HygieneDecodeContext, HygieneEncodeContext, SyntaxContextData},
        source_map::SourceMap,
        BytePos, CachingSourceMapView, ExpnData, ExpnHash, ExpnId, Pos, RelativeBytePos,
        SourceFile, Span, SpanDecoder, SpanEncoder, StableSourceFileId, Symbol, SyntaxContext,
    };
    ...
    // Adapted from compiler/rustc_middle/src/query/on_disk_cache.rs:223
    pub fn save_to_encoder<'tcx>(
        tcx: TyCtxt<'tcx>,
        encoder: FileEncoder,
        item: &impl for<'a> Encodable<CacheEncoder<'a, 'tcx>>,
    ) -> FileEncodeResult {
        // Determine source file mapping
        ...
        // Encode the file's main contents
        item.encode(&mut encoder);

        // Encode allocation data
    ...
    pub fn load_from_decoder<'tcx, D>(tcx: TyCtxt<'tcx>, data: Mmap, start_pos: usize) -> D
    where
        D: for<'a> Decodable<CacheDecoder<'a, 'tcx>>,
    {
        ...
        let mut decoder = CacheDecoder {
            tcx,
            opaque: MemDecoder::new(&data, pos.to_usize()),
            source_map: tcx.sess.source_map(),
            file_index_to_file: &Default::default(),
            file_index_to_stable_id: &footer.file_index_to_stable_id,
            alloc_decoding_session: alloc_decoding_session.new_decoding_session(),
    ...
    impl AbsoluteBytePos {
        ...
        fn new(pos: usize) -> AbsoluteBytePos {
            AbsoluteBytePos(
                pos.try_into()
                    .expect("Incremental cache file size overflowed u64."),
            )
        }
    ...
    impl EncodedSourceFileId {
        #[inline]
        pub fn new(tcx: TyCtxt<'_>, file: &SourceFile) -> EncodedSourceFileId {
            EncodedSourceFileId {
    ...
    //- DECODING -------------------------------------------------------------------
    // Adapted from compiler/rustc_middle/src/query/on_disk_cache.rs:453
    ...
    impl<'a, 'tcx> SpanDecoder for CacheDecoder<'a, 'tcx> {
        ...
        fn decode_expn_id(&mut self) -> ExpnId {
            ...
            let krate = self
                .tcx
                .stable_crate_id_to_crate_num(hash.stable_crate_id());
            ...
            let expn_id = if krate == LOCAL_CRATE {
                ...
                let data: ExpnData = self.with_position(pos.to_usize(), |decoder| {
                    decode_tagged(decoder, TAG_EXPN_DATA)
                });
        ...
        fn decode_span(&mut self) -> Span {
            ...
            if tag == TAG_PARTIAL_SPAN {
                ...
                let enclosing = self
                    .tcx
                    .source_span_untracked(parent.unwrap())
                    .data_untracked();
                let span = Span::new(
    ...
    impl<'a, 'tcx> Decodable<CacheDecoder<'a, 'tcx>>
        for &'tcx rustc_middle::traits::specialization_graph::Graph
    {
        #[inline]
        fn decode(d: &mut CacheDecoder<'a, 'tcx>) -> Self {
            RefDecodable::decode(d)
    ...
    //- ENCODING -------------------------------------------------------------------
    // Adapted from compiler/rustc_middle/src/query/on_disk_cache.rs:820
    ...
    impl<'a, 'tcx> SpanEncoder for CacheEncoder<'a, 'tcx> {
        fn encode_syntax_context(&mut self, syntax_context: SyntaxContext) {
            rustc_span::hygiene::raw_encode_syntax_context(
                syntax_context,
